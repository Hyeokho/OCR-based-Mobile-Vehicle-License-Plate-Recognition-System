...
def save_info(choice):
	#선불 A주차장 지역 지정 선불B주차장 일 경우 prepay_B로 변경
	zone = 'prepay_A'
	if choice == 1:
		if myid.get() == '' :
		...
		if myname.get() == '':
			...
			if yes_dp.get() == 1:
				dev_mysql.prepay_in_car_parking(str.get(), myname.get(), myphone.get(), yes_dp.get(), zone)
			elif no_dp.get() == 1:
				dev_mysql.prepay_in_car_parking(str.get(), myname.get(), myphone.get(), yes_dp.get(), zone)
...
but2=Button(frame,width=23,height=4,bg='dim gray',fg='white',bd=0,relief=GROOVE,command=lambda:capture(2),text='입차',font=('helvetica 15 bold'))
