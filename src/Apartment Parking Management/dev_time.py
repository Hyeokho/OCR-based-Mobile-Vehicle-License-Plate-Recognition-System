...
# 2017-08-07 13:02 처럼 현재 시간에서 5분 후에 시간을 문자열로 리턴
# beforeMin 에는 이전 몇 분이 들어간다. 예) getCurrentStrMinTimeAgo(5)
def getCurrentStrMinTimLater(afterMin):
   return (datetime.datetime.now() + datetime.timedelta(minutes=afterMin)).strftime("%Y-%m-%d %H:%M" + ":00")
